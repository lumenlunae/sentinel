defmodule Html.AuthControllerTest do
  ...
  test "sign in with unknown email", %{conn: conn} do
    conn =
      post(conn, auth_path(conn, :create), %{
        session: %{email: @unknown_email, password: @password}
      })
    ...
    assert String.contains?(
             conn.resp_body,
             Sentinel.Config.router_helper().auth_path(Sentinel.Config.endpoint(), :new)
           )
  ...
  test "sign in with wrong password", %{conn: conn, params: params} do
    conn =
      post(conn, auth_path(conn, :create), %{
        session: %{password: "wrong", email: params.session.email}
      })
