defmodule Html.PasswordControllerTest do
  ...
  setup do
    ...
    {:ok, token, claims} = Sentinel.Guardian.encode_and_sign(user, permissions)

    authenticated_conn =
      build_conn()
      |> Plug.Test.init_test_session(%{guardian_default_token: token})
  ...
  test "request a reset token", %{conn: conn, user: user} do
    ...
    with_mock Sentinel.Mailer, [:passthrough],
      send_password_reset_email: fn _, _ -> mocked_mail end do
  ...
  test "reset password with a wrong token", %{conn: conn, user: user, auth: auth} do
    ...
    assert String.contains?(
             conn.resp_body,
             Sentinel.Config.router_helper().password_path(Sentinel.Config.endpoint(), :new)
           )
  end
  ...
  test "Reset password when logged in", %{authenticated_conn: conn, user: user, auth: auth} do
    ...
    conn =
      put(conn, password_path(conn, :authenticated_update), %{
    ...
    {:ok, _} =
      Ueberauthenticator.ueberauthenticate(%Ueberauth.Auth{
        provider: :identity,
        uid: user.email,
