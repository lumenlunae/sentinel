defmodule Html.UserControllerTest do
  ...
  setup do
    ...
    welcome_email =
      Sentinel.Mailer.send_welcome_email(
        %Sentinel.User{
    ...
    invite_email =
      Sentinel.Mailer.send_invite_email(
        %Sentinel.User{
          email: params.user.email,
          id: 1
        },
  ...
  test "default sign up", %{conn: conn, params: params, welcome_email: mocked_mail} do
    ...
    with_mock Sentinel.Mailer, [:passthrough], send_welcome_email: fn _, _ -> mocked_mail end do
      ...
      assert String.contains?(
               conn.private.phoenix_flash["info"],
               "You will receive an email with instructions for how to confirm your email address in a few minutes."
             )

      assert String.contains?(
               conn.resp_body,
               Sentinel.Config.router_helper().account_path(Sentinel.Config.endpoint(), :edit)
             )
  ...
  # green
  test "inviting a user via the invitable sign up", %{
    conn: conn,
    invite_params: params,
    invite_email: mocked_mail
  } do
    Config.persist(sentinel: [invitable: true])
    Config.persist(sentinel: [confirmable: false])

    with_mock Sentinel.Mailer, [:passthrough], send_invite_email: fn _, _ -> mocked_mail end do
      token_count = length(TestRepo.all(Token.query_schema()))
      conn = post(conn, auth_path(conn, :callback, "identity"), params)
      response(conn, 302)

      refute token_count + 1 == length(TestRepo.all(Token.query_schema()))
      assert_delivered_email(mocked_mail)

      assert String.contains?(
               conn.resp_body,
               Sentinel.Config.router_helper().user_path(Sentinel.Config.endpoint(), :new)
             )
    end
  end

  # green
  test "invitable and confirmable sign up", %{
    conn: conn,
    invite_params: params,
    invite_email: mocked_mail
  } do
  ...
  test "invitable setup password", %{conn: conn, params: params} do
    ...
    conn =
      put(conn, user_path(conn, :invited, user.id), %{
    ...
    assert token_count + 1 == length(TestRepo.all(Token.query_schema()))
  end

  test "sign up with missing password without the invitable module enabled", %{
    conn: conn,
    invite_params: params
  } do
    Config.persist(sentinel: [invitable: false])
  ...
  test "sign up with missing email", %{conn: conn} do
    conn =
      post(conn, auth_path(conn, :callback, "identity"), %{"user" => %{"password" => @password}})

    # FIXME or 422?
    response(conn, 401)

    # FIXME is this the error message we want?
    assert String.contains?(conn.private.phoenix_flash["error"], "Failed to authenticate")
  end
  ...
  test "sign up with a custom registrator callback", %{conn: conn, params: params} do
    Config.persist(
      sentinel: [registrator_callback: {Sentinel.TestRegistratorCallback, :registrator_callback}]
    )
