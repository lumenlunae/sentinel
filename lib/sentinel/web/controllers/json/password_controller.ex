  ...
  def update(conn, params = %{"user_id" => user_id}, _headers, _session) do
    ...
    case Sentinel.Update.update_password(user_id, params) do
      {:ok, _auth} ->
        ...
        case Sentinel.Guardian.encode_and_sign(user, permissions) do
          {:ok, token, _encoded_claims} ->
            json(conn, %{token: token})

          {:error, :token_storage_failure} ->
            Util.send_error(conn, %{
              errors: "Failed to store session, please try to login again using your new password"
            })

          {:error, reason} ->
            Util.send_error(conn, %{errors: reason})
        end
