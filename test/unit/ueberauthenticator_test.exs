defmodule UeberauthenticatorTest do
  ...
  test "identity provider without user or passsword" do
    assert {:error, [email: {"can't be blank", [validation: :required]}]} =
  ...
  test "identity provider without password, invitable false", %{user: user} do
    ...
    assert {:error, [password: {"A password is required to login", []}]} =
             Ueberauthenticator.ueberauthenticate(%Auth{
               provider: :identity,
  ...
  test "create new user, identity provider without password, invitable true" do
    ...
    assert {:ok, %{user: _user = %Sentinel.User{}, confirmation_token: _confirmation_token}} =
             Ueberauthenticator.ueberauthenticate(%Auth{
               provider: :identity,
               info: %Auth.Info{email: user.email},
               credentials: %Auth.Credentials{
  ...
  test "identity provider without email", %{auth: auth} do
    assert {:error, [email: {"An email is required to login", []}]} =
             Ueberauthenticator.ueberauthenticate(%Auth{
               provider: :identity,
               credentials: %Auth.Credentials{
                 other: %{
  ...
  test "identity provider with password, unknown user", %{auth: auth} do
    assert {:error, [base: {"Unknown email or password", []}]} =
             Ueberauthenticator.ueberauthenticate(%Auth{
               provider: :identity,
               credentials: %Auth.Credentials{
                 other: %{
  ...
  test "identity provider with email and pass, user exists, auth exists, is successful", %{
    user: user,
    auth: auth
  } do
    assert {:ok, _user = %Sentinel.User{}} =
             Ueberauthenticator.ueberauthenticate(%Auth{
               provider: :identity,
               credentials: %Auth.Credentials{
                 other: %{
  ...
  test "identity provider with password and confirmation, user DOESN'T exist, auth DOESN'T, REGISTERABLE false - doesn't create a new user" do
    ...
    assert {:error, [base: {"New user registration is not permitted", []}]} =
             Ueberauthenticator.ueberauthenticate(%Auth{
               provider: :identity,
               info: %Ueberauth.Auth.Info{
                 email: "test@example.com"
  ...
  test "authenticate an unconfirmed user - confirmable default/optional", %{
    user: user,
    auth: auth
  } do
    Config.persist(sentinel: [confirmable: :optional])
