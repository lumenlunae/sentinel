defmodule Json.AuthControllerTest do
  ...
  test "sign in with unknown email", %{conn: conn} do
    conn =
      post(conn, auth_path(conn, :create), %{user: %{email: @unknown_email, password: @password}})
  ...
  test "sign in with wrong password", %{conn: conn, params: params} do
    conn =
      post(conn, auth_path(conn, :create), %{user: %{password: "wrong", email: params.user.email}})
  ...
  test "sign in as unconfirmed user - confirmable default/optional", %{conn: conn, params: params} do
    ...
    TestRepo.get_by!(Token.query_schema(), jwt: token)
  end
  ...
  test "sign in as unconfirmed user - confirmable required", %{conn: conn, params: params} do
    ...
    assert response == %{
             "errors" => [
               %{
                 "base" =>
                   "Account not confirmed yet. Please follow the instructions we sent you by email."
               }
             ]
           }
  end
  ...
  test "sign out", %{conn: conn} do
    ...
    {:ok, token, _} = Sentinel.Guardian.encode_and_sign(user, permissions)
